package dev.hyzoon.oauth_test.api.controller.auth;

import dev.hyzoon.oauth_test.api.service.auth.AuthService;
...
import dev.hyzoon.oauth_test.util.CookieUtil;
...
public class AuthController {
    ...
    @PostMapping("/token")
    public ResponseEntity<TokenDto> exchangeToken(@RequestBody Map<String, String> payload, HttpServletResponse response) {
        String code = payload.get("code");
        TokenDto tokenDto = authService.exchangeCodeForToken(code);

        // Refresh Token은 HttpOnly 쿠키로 설정
        int cookieMaxAgeSeconds = (int) (jwtProperties.getRefreshTokenExpiry() / 1000);
        CookieUtil.addCookie(response, OAuth2AuthenticationSuccessHandler.REFRESH_TOKEN_COOKIE_NAME, tokenDto.getRefreshToken(), cookieMaxAgeSeconds);
    ...
    public ResponseEntity<TokenDto> refreshToken(
        ...
        CookieUtil.addCookie(response, OAuth2AuthenticationSuccessHandler.REFRESH_TOKEN_COOKIE_NAME, newTokenDto.getRefreshToken(), cookieMaxAgeSeconds);
